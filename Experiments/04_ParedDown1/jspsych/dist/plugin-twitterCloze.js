var jsPsychTwitterStaticCloze = (function (jspsych) {
    ...
    class TwitterStaticClozePlugin {
      ...
      trial(display_element, trial) {
        ...
        // Replace the cloze word with an inline input once
        let replaced = false;
        let tweetHtml = trial.tweet_text.split(/\s+/).map(w => {
          ...
          if (!replaced && clean === trial.cloze_word.replace('#', '').toLowerCase()) {
            ...
            const estWidth = Math.max(60, trial.cloze_word.length * 10);
        ...
        submitBtn.addEventListener('click', () => {
          ...
          const correctAnswer = (trial.cloze_word).trim().replace('#', '');
          const isCorrect = response.toLowerCase() === correctAnswer.toLowerCase();
  
          if (isCorrect) {
            inputEl.style.color = 'rgb(99, 196, 99)'; // green
          } else {
            inputEl.style.color = 'rgb(225, 119, 119)'; // red
            setTimeout(() => {
              inputEl.value = correctAnswer;
              inputEl.style.color = 'rgb(99, 196, 99)';
            }, 1000);
          }
  
          setTimeout(() => {
            this.end_trial(trial, {
              response,
            //   correct: isCorrect,
