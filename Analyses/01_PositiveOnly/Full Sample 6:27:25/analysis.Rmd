...
```{r}
politicalCounts <- token_lookup %>% 
  select(workerid, ProlificPolitical) %>% 
  unique() %>% 
...
```{r}
lexicalDecisionForModeling <- lexicalDecisionFull %>% 
  ...
  filter(ProlificPolitical.x != "CONSENT_REVOKED")

rtModel <- brm(
  rt ~ exposure * alignment + (1 | workerid) + (1 | stimulus),
  data = lexicalDecisionForModeling,
  family = lognormal(),
...
prodCodedForModel <- prodCodedFull %>% 
  mutate(exposure = ifelse(tokenCount == 8, 'high','low'))
...
```{r}
prod_table <- tidy(fit_full, effects = "fixed") %>%
  select(term, estimate, std.error, conf.low, conf.high) %>%
  rename(
    Term = term,
    Estimate = estimate,
...
prod_table %>%
  ...
  tab_header(
    ...
    subtitle = "Predictors of term selection"
  ) %>%
  fmt_number(
    columns = c(Estimate, SE, `95% CI (Lower)`, `95% CI (Upper)`),
    decimals = 3
...
gtsave(prod_table %>% gt(), "prodModel_table.html")
```
