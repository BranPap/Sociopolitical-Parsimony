...
```{r}
leftWingData <- read_csv("sociopoliticalfrequencypilot2left-merged.csv")

rightWingData <- read_csv("sociopoliticalfrequencypilot2right-merged.csv")
...
allData %>% 
  filter(category=="LexicalDecision") %>% 
  mutate(binary_response = ifelse(statusCheck == "correct",1,0)) %>% 
  group_by(workerid,poli) %>% 
  summarize(
    meanAcc = mean(binary_response)
  ) %>% 
  ggplot(aes(x=workerid, y=meanAcc)) + 
